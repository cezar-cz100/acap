...
////////////////////////////////////////////////////////////////////////////////
    ...
    extend(string, true, true, 
    {
        ...
        naturalCompare: function( b )
        {
            ...
            comparison = collateStrings( a, b );
            
            return comparison;
        }
    });
    
    extend(string, false, true, 
    {
        pluralize: function( count, singular, plural )
        {
            ...
            else if( count != 0 && ( count % 100 >= 20 || count % 100 == 0 ) )
            {
                text += 'de ' + plural;
            }
            else
        ...
        ordinal: function( order, genFeminin )
        {
            var text = '';
            
            order = Number.toNumber( order );
    ...
    extend(object, false, true, 
    {
        isScalar: function( v )
        {
            return object.isNumber( v ) || object.isString( v );
        },
        
        isEnumerable: function( v )
        {
            return object.isArray( v ) || object.isObject( v );
        },
        
        extend: function( obj, v )
        {
            return object.merge( obj, v );
        },
        
        join: function( a, b )
        {
            var x = object.merge( {}, a ),
                y = object.extend( x, b );
        ...
        makeObject: function( v )
        {
            if( !object.isObject( v ) ) v = {};
            return v;
        },
        ...
        isBlank: function( value )
        {
            return ( 
                    this.isEmpty( value ) 
                    || ( this.isString( value ) && value.isBlank() ) 
                    || value === 0 
            );
        },
        ...
        equal: function( a, b )
        {
            ...
            {
                return a.subtract( b ).length == 0 && b.subtract( a ).length == 0;
            }
            else
            {
                return _equal( a, b );
    ...
    var _sortBy = array.prototype.sortBy;

    extend(array, true, true, {
        valuesFromKey: function( k ){
            var values = [];
            this.each(function( element ){
                values.push( element[ k ] );
            });
        ...
        contains: function( value, strict )
        {
            if( strict === undefined )
            {
                strict = false;
            ...
            else
            {
                return this.findIndex( function( element ) { 
                    if( Object.isScalar( value ) && Object.isScalar( element ) )
                    {
                        return value.toString() === element.toString();
                    }
                    else
                    {
                        return Object.equal( element, value );
                    }
                } ) >= 0;
            }
        }
    ...
    extend(Function, true, true, {
        ...
        'methodProxy': function( obj, methodName ){
            return $.proxy( obj[ methodName ], obj );
        }
    });
    
    extend(date, false, true, {
        getMonthName: function( monthIndex, year ){
            return $.datepicker._defaults.monthNames[ monthIndex - 1 ] + ( year ? ( ' ' + year ) : '' );
        },
        getMonths: function(){
            var months = {}, index = 1;
            $.datepicker._defaults.monthNames.each( function( monthName ){
        ...
        getCurrentYear: function(){
            return Date.create().format('{year}');
        },
        getCurrentMonth: function(){
            return Date.create().format('{M}');
        },
        format: function( format, dateValue ){
            return $.datepicker.formatDate( format, new Date( dateValue ) );
        }
    });
    
    extend(date, true, true, {
        formatISO: function(){
            return this.format( Date.ISO8601_DATETIME );
        }
    });
    ...
    extend(number, false, true, 
    {
        ...
        {
            return n.toNumber().toString() === n.toString().trim();
        },
        
        validateFloat: function( n ) 
        {
            var stringValue = n.toString().trim();
            ...
            check_char = start_format.indexOf( stringValue.charAt( 0 ) );
            ...
            {
                check_char = number_format.indexOf( stringValue.charAt( i ) )
                if ( check_char < 0 )
                {
                    return false;
                }
        ...
        formatCurrency: function( floatValue, precision, padWithZeros )
        {
            var numericValue = floatValue.toNumber();
            ...
            else
            {
                ...
                return numericValue.round( precision ).format( Object.isEmpty( padWithZeros ) ? undefined : precision , '.', ',' );
            }
        },
        
        valueFromCurrency: function( currency )
        {
            var stringValue = currency.toString().trim().replace( ',', '.' ),
                floatValue = stringValue.isBlank() ? 
                        0 : 
                        number.validateFloat( stringValue ) ?
        ...
        {
            var numericValue = this.toNumber( value );
            
            return this.isNaN( numericValue ) ? '' : numericValue.format( undefined, '', ',' );
        },
        ...
        toNumber: function( value )
        {
            ...
            else if( Object.isEnumerable( value ) )
            {
                number = Object.size( value );
            }
